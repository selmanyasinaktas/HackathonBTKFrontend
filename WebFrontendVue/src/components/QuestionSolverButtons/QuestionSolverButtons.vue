<template>
    <div>
      ...
      <div v-if="isCameraOpen" class="fixed inset-0 bg-gray-800 bg-opacity-75 flex items-center justify-center z-50">
        <div class="light:bg-white dark:bg-gray-800 p-14 rounded-lg shadow-lg relative w-11/12 h-5/6 md:w-9/12 md:h-5/6">
          ...
          <button @click="captureImage" class="px-4 py-2 bg-blue-500 text-white rounded-lg">Fotoğraf Çek</button>
        </div>
      </div>
      ...
      <!-- Metin Ekleme Modalı -->
      <div v-if="isTextModalOpen" class="fixed inset-0 bg-gray-800 bg-opacity-75 flex items-center justify-center z-50">
        <div class="light:bg-white dark:bg-gray-800 p-14 rounded-lg shadow-lg relative w-11/12 h-5/6 md:w-9/12 md:h-5/6">
          ...
          <VueQuill v-model:content="richTextContent" />
          <div class="flex mt-4">
  ...
  import VueQuill from '../quill/VueQuill.vue';
  ...
  <style scoped>
  /* Kırpma modalı ve diğer özel stiller */
  </style>
